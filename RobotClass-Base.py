...
class ur5_rf85:
    """Robot ur5 + gripper rf85 class"""

    def __init__(self, urdf_root=pybullet_data.getDataPath(), root="", robotgripper_urdf="urdf/ur5_rf85.urdf",
                 last_robot_joint_name="wrist_3_joint", urdf_error_correct_orien_e=[1.57, 0.0, 0.0],
                 tcp_offset_pos=[0.0, 0.0, 0.260], tcp_offset_orien_e=[3.14, 0.0, 0.0],
                 robot_control_joints=["shoulder_pan_joint", "shoulder_lift_joint",
                                       "elbow_joint", "wrist_1_joint", "wrist_2_joint", "wrist_3_joint"],
                 gripper_main_control_joint="robotiq_85_left_knuckle_joint",
                 mimic_joint_name=["robotiq_85_right_knuckle_joint",
                                   "robotiq_85_left_inner_knuckle_joint",
                                   "robotiq_85_right_inner_knuckle_joint"],
                 mimic_multiplier=[1, 1, 1, -1, -1],
                 gripper_fingers_control_name=["robotiq_85_right_finger_tip_z", "robotiq_85_right_finger_tip_y",
                                               "robotiq_85_left_finger_tip_z", "robotiq_85_left_finger_tip_y"],
                 home=[-0.992, -1.157, 1.323, -1.720, -1.587, 0.0],
                 visual_inspection=True):
        """Initialization function
        urdf_root (str): root for URDF objects
        root (str): root directory for local files
        robotgripper_urdf (str): name and location of robot+gripper URDF file
        last_robot_joint_name (str): URDF name of the joint to connect the gripper
        urdf_error_correct_orien_e (list): Orientation correction if needed due to urdf error
        tcp_offset_pos (list): position offset referent to the last_robot_joint
        tcp_offset_orien_e (list): orientation offset referent to the last_robot_joint
        robot_control_joints (list): named robot control joints in right order for inverse kinematics
        gripper_main_control_joint (str): main control joint name in the URDF
        mimic_joint_name (list): URDF names of joints which moves at the same time and proportional to gripper_main_control_joint
        mimic_multiplier (list): proportional value of angle variation for each angle modified in gripper_main_control_joint
        gripper_fingers_control_name (list): names to control prismatic joints finger tips
        home (list): joint angles in radiants of the initial joint position, I was also useing it to find solution near this configuration
        visual_inspection (bool): flag sleep every simulation step for real-time like visualizations
        """
        self.urdf_root = urdf_root
        self.robotgripper_urdf = os.path.join(root, robotgripper_urdf)
